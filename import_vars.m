...
function import_vars(vrs,dpath,source,yr_end,pred_vars_arc,cmems)
    ...
    import_SSS(dpath,vrs,source.SSS,lat,lon,time,yr_end,cmems,'plot_option',1);

    % 2. obtain sea surface height (Options: 'CMEMS', 'NASA')
    import_SSH(dpath,vrs,source.SSH,lat,lon,time,yr_end,cmems,'plot_option',1);

    % 3. obtain sea surface temperature
    import_SST(dpath,vrs,source.SST,lat,lon,time,yr_end,'plot_option',1);

    % 4. obtain sea surface ice concentration
    import_IceC(dpath,vrs,source.IceC,lat,lon,time,yr_end,ocean_mask,'plot_option',1);
    ...
    import_Wind(dpath,vrs,source.Wind,lat,lon,time,yr_end,'plot_option',1);
    ...
    import_MSLP(dpath,vrs,source.MSLP,lat,lon,time,yr_end,'plot_option',1);

    % 9. obtain atmospheric pCO2
    import_apCO2(dpath,vrs,source.apCO2,lat,lon,time,yr_end,...
        source.MSLP,source.SSS,source.SST,'plot_option',1);

    % 10. obtain bathymetry from ETOPO2
    import_Bathy(dpath,vrs,source.Bathy,lat,lon,'plot_option',1);

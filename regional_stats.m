function regional_stats(vrs,lme_shape,lme_idx,region,yr_end)

% region labels
reg_lab = {'CCS' 'GA' 'AI' 'EBS' 'BS' 'NBCS' 'NE' 'SE' 'GM' 'CS' 'PI'};
% variable information
var_type = {'DIC' 'pCO2' 'fCO2' 'TA' 'pH' 'OmA' 'OmC' 'H' 'CO3' 'RF'};
var_type_2 = {'SST' 'SSS' 'TA_DIC' 'SSH' 'CHL' 'WindSpeed' 'MLD'};
...
for n = 1:length(region)
    ...
    for var_num = 1:length(var_type)
        ...
        area_weights = LME.area_km2.*LME.percent_sea;
        area_weights = repmat(area_weights,1,1,12);
        % establish climatological index of only cells that are always uncovered
        spatial_index = nan(RFR_LME.dim.x,RFR_LME.dim.y,12);
        for m = 1:12
            spatial_index(:,:,m) = ...
                sum(RFR_LME.idxspc(:,:,m:12:end),3) == ...
                    RFR_LME.dim.z/12;
        end
        ...
        region_percent = 100.*(sum(reshape(area_weights_masked,...
            [RFR_LME.dim.x.*RFR_LME.dim.y 12]),'omitnan')./...
            sum(reshape(area_weights,...
            [RFR_LME.dim.x.*RFR_LME.dim.y 12]),'omitnan'));
        % replicate area weights and index over time
        area_weights_masked = repmat(area_weights_masked,1,1,RFR_LME.dim.z/12);
        spatial_index = repmat(spatial_index,1,1,RFR_LME.dim.z/12);

        % where RFR_LME is nan but area mask is not
        sum(sum(isnan(RFR_LME.(var_type{var_num})(:,:,1)) & ~isnan(area_weights_masked(:,:,1))))

        % calculate area-weighted time series
        RFR_LME.var_dom_mean = nan(RFR_LME.dim.z,1);
        RFR_LME.u_var_dom_mean = nan(RFR_LME.dim.z,1);
        for t = 1:RFR_LME.dim.z
            % calculate area-weighted means
            if any(any(~isnan(area_weights_masked(:,:,t))))
                if strcmp(var_type{var_num},'SST') || strcmp(var_type{var_num},'SSS') || ...
                   ...
                   strcmp(var_type{var_num},'WindSpeed') || strcmp(var_type{var_num},'MLD')
                    ...
                    RFR_LME.var_dom_mean(t) = ...
                        squeeze(sum(sum(LME.(var_type{var_num})(:,:,t).*...
                            area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'))./...
                            squeeze(sum(sum(area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'));
                    RFR_LME.u_var_dom_mean(t) = NaN;
                elseif strcmp(var_type{var_num},'TA_DIC')
                    RFR_LME.var_dom_mean(t) = ...
                        squeeze(sum(sum((RFR_LME.TA(:,:,t)./...
                        RFR_LME.DIC(:,:,t)).*...
                            area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'))./...
                            squeeze(sum(sum(area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'));
                    RFR_LME.u_var_dom_mean(t) = NaN;
                else
                    % RFR_LME.(var_type{var_num})(~spatial_index) = NaN;
                    RFR_LME.var_dom_mean(t) = ...
                        squeeze(sum(sum(RFR_LME.(var_type{var_num})(:,:,t).*...
                            area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'))./...
                            squeeze(sum(sum(area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'));
                    RFR_LME.u_var_dom_mean(t) = ...
                        squeeze(sum(sum(RFR_LME.(['u' var_type{var_num}])(:,:,t).*...
                            area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'))./...
                            squeeze(sum(sum(area_weights_masked(:,:,t),1,'omitnan'),2,'omitnan'));                
                end
            else
                RFR_LME.var_dom_mean(t) = NaN;
                RFR_LME.u_var_dom_mean(t) = NaN;
            end
...
%             end
        ...
        if strcmp(var_type{var_num},'H')
            RFR_LME.var_dom_mean = (10^9).*RFR_LME.var_dom_mean;
            RFR_LME.u_var_dom_mean = (10^9).*RFR_LME.u_var_dom_mean;
        end

        % re-calculate time
        time = datenum([RFR_LME.year ...
                RFR_LME.month_of_year ...
                repmat(15,RFR_LME.dim.z,1)]);

        % calculate long-term mean
        mean_lt = mean(RFR_LME.var_dom_mean,'omitnan');

        % calculate trend
        [yf,RFR_LME.var_dom_mean_anom,x,err] = ...
            leastsq2(RFR_LME.month,...
            RFR_LME.var_dom_mean,0,3,[4 6 12]);
        tr = x(2)*12;

        % uncertainty on trend
        [acov,acor,lag,dof] = ...
            autocov2(RFR_LME.month,...
            RFR_LME.var_dom_mean_anom,36);
        % plot(lag,acor)
        edof = dof - 8; % subtract number of parameters to get effective dof
        edof(edof<1) = 1;
        tr_uncer = ... % scale uncertainty using edof
            err(2)*(sqrt(length(RFR_LME.month))./sqrt(edof))*12;
    
        % calculate interannual variability
        iav = std(RFR_LME.var_dom_mean_anom);
        ...
        clim_fit = nan(12,1);
        for m = 1:12
            clim(m) = mean(RFR_LME.var_dom_mean(m:12:end),'omitnan');
            clim_uncer(m) = mean(RFR_LME.u_var_dom_mean(m:12:end),'omitnan');
        ...
        % Test plots
        % figure; plot(RFR_LME.month,yf); hold on;
        % scatter(RFR_LME.month,RFR_LME.var_dom_mean);  hold off;
        % figure; plot(1:12,clim); hold on; plot(1:12,clim_fit); hold off;
        % close all

        % calculate annual means
        time_ann = nan(length(RFR_LME.year)/12,1);
        RFR_LME.var_dom_mean_ann = nan(length(RFR_LME.year)/12,1);
        RFR_LME.u_var_dom_mean_ann = nan(length(RFR_LME.year)/12,1);
        for y = 1:length(RFR_LME.year)/12
            time_ann(y) = ...
                mean(time((y-1)*12+1:(y-1)*12+12));
            RFR_LME.var_dom_mean_ann(y) = ...
                mean(RFR_LME.var_dom_mean((y-1)*12+1:(y-1)*12+12),'omitnan');
            RFR_LME.var_dom_mean_ann_anom(y) = ...
                mean(RFR_LME.var_dom_mean_anom((y-1)*12+1:(y-1)*12+12),'omitnan');
            RFR_LME.u_var_dom_mean_ann(y) = ...
                mean(RFR_LME.u_var_dom_mean((y-1)*12+1:(y-1)*12+12),'omitnan');
        end
% 
%         % calculate trend over most recent 5 years
%         [yf5,yr5,x5,err5] = ...
%             leastsq2(RFR_LME.month(end-12*5:end),...
%             RFR_LME.var_dom_mean(end-12*5:end),0,2,[6 12]);
%         pos_signif_5 = x5(2)-err5(2) > 0;
%         neg_signif_5 = x5(2)+err5(2) < 0;
%         % calculate mean over most recent 5 years
%         meantot = mean(RFR_LME.var_dom_mean);
%         conf90 = std(RFR_LME.var_dom_mean)*1.645;
%         mean5 = mean(RFR_LME.var_dom_mean(end-12*5:end));
...
%     
        ...
        monthly(:,(n-1)*length(var_type)+var_num) = RFR_LME.var_dom_mean;
        monthly_anom(:,(n-1)*length(var_type)+var_num) = RFR_LME.var_dom_mean_anom;
        u_monthly(:,(n-1)*length(var_type)+var_num) = RFR_LME.u_var_dom_mean;

        % fill table with annual values
        ann(:,(n-1)*length(var_type)+var_num) = RFR_LME.var_dom_mean_ann;
        ann_anom(:,(n-1)*length(var_type)+var_num) = RFR_LME.var_dom_mean_ann_anom;
        u_ann(:,(n-1)*length(var_type)+var_num) = RFR_LME.u_var_dom_mean_ann;
        ...
        set(gca,'FontSize',12);
        % plot
        plot(time,RFR_LME.var_dom_mean,'k-','linewidth',1);
        plot(time_ann,RFR_LME.var_dom_mean_ann,'r-','linewidth',3);
        fill([time;flipud(time)],[RFR_LME.var_dom_mean+RFR_LME.u_var_dom_mean;...
            flipud(RFR_LME.var_dom_mean-RFR_LME.u_var_dom_mean)],'k',...
            'FaceAlpha',0.2,'LineStyle','none');
        fill([time_ann;flipud(time_ann)],[RFR_LME.var_dom_mean_ann+RFR_LME.u_var_dom_mean_ann;...
            flipud(RFR_LME.var_dom_mean_ann-RFR_LME.u_var_dom_mean_ann)],'k',...
            'FaceColor','r','FaceAlpha',0.2,'LineStyle','none');
%         fill([time_ann;flipud(time_ann)],[RFR_LME.var_dom_mean_ann+RFR_LME.u_var_dom_mean_ann;...
%             flipud(RFR_LME.var_dom_mean_ann-RFR_LME.u_var_dom_mean_ann)],'r',...
%             'FaceAlpha',0.2,'LineStyle','none');
        scatter(time_ann,RFR_LME.var_dom_mean_ann,200,'k.');
        datetick('x');
        % plot means
        plot([time(1) time(end)],[mean_lt mean_lt],'k--','linewidth',2);
        % plot([time(end-12*5) time(end)],[mean5 mean5],'k--','linewidth',2);
        % add text to plot
        title([reg_lab{n} ' | \mu = ' num2str(round(mean(RFR_LME.var_dom_mean,'omitnan'),rounder(var_num))) ...
...
%         end
        ...
        % add text to plot
        title([reg_lab{n} ' | \mu = ' num2str(round(mean(RFR_LME.var_dom_mean),rounder(var_num))) ...
            ' ' units{var_num} ' | Amp. = ' ...
            num2str(round(amp,rounder(var_num))) ' ' units{var_num}],...
...
%         end
        ...
        if ~isfolder(['IndsAndStats/' region{n}])
            mkdir(['IndsAndStats/' region{n}]);
        end
        mnth_tmp = [RFR_LME.year,RFR_LME.month_of_year,...
            RFR_LME.var_dom_mean,RFR_LME.u_var_dom_mean];
        ...
        ann_tmp = [unique(RFR_LME.year),...
            RFR_LME.var_dom_mean_ann,RFR_LME.u_var_dom_mean_ann];
...
for var_num = 1:length(var_type)
    if isfile(['IndsAndStats/' var_type{var_num} '.csv'])
